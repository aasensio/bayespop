...
!-----------------------------------------------------------------------
! Solve the optimization problem using SCALCG
!-----------------------------------------------------------------------
subroutine maximumLikelihoodScalCG(sdim, x)
integer :: sdim
real(kind=8) :: x(sdim)
...
! Optimize the merit function using SCAL-CG
		call scalcg(sdim,x,epsg,epsf,delta,maxiter,stoptest,fx,gnorm, &
			iter,irstart,igamma,fgcnt,lscnt,tetas,tetaa, &
			iprint, 1)
	
end subroutine maximumLikelihoodScalCG
